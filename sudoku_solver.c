...
/*
 *  Function: File 2 sudoku                                      
 *  Imput arguments: 
 *  int *sudoku : pointer where the numbers of the sudoku will be stored
 *  char *filename : name of the file where the sudoku is stored
 *  Any error (number of characters not equal to 81, or characters that are not numbers)
 *  will give 1 as return.
 */
int file_2_sudoku(int *sudoku, char *filename)
{
  FILE *fp;
  ...
  // Error. There was a non numeric character, or there weren't 81 numbers
  if ((i!=82) || (error=1))
    return 1;
  else
    return 0;
 ...
 *  Checks the given file of the sudoku looking for repetitions
 *  Imput arguments: 
 *   int *sudoku : pointer where the sudoku is stored
 *   int file : file that we want to check
 *  Return:
